<?php
	...
	$allHttpTestPlotName = CDBHttpTestPlot::getAllHttpDLTestPlotNameByUserId();
	
	$allHttpTestPlotName = array("" => "Please select") + $allHttpTestPlotName;
	
	$form_contents = new Form("resultStatisticsForm");
	$form_contents->configure(array("action" => "httpDLTestPlotResultStat.php", "method" => "post"));

	$form_contents->addElement(
		new Element_HTMLExternal(
			'<errormsg for="plotName" name="checkHttpDLPlotAvailable">Please select a valid download test plot</errormsg>
	...
	$form_contents->addElement(
		new Element_Select(
			...
			, array(
				"id"=>"plotName", "validate"=>'{"checkHttpDLPlotAvailable":"checkHttpDLPlotAvailable"}',
