<?php
	...
	$MY_AVAILABLE_SERVICE = CDBUser::getMachineDataServiceACL($COCKPIT_SYSTEM_DEF['userId']);
	
	/********************* Reseource Allocation ****************************/

	// Rearrange Employer dashboard according to their change of widget
	$quickLinkColumnsRearrange = CDBUser::rearrangeMachineDataDashboard($COCKPIT_SYSTEM_DEF['userId'], array_keys($MY_AVAILABLE_SERVICE));
	...
	reset($MACHINEDATA_CONTROL_PANEL);
	
	foreach($MACHINEDATA_CONTROL_PANEL as $section => $section_text)
	{
		...
		foreach($MACHINEDATA_CONTROL_PANEL_SUB[$section] as $content_link => $content_text)
		{
	...
	$js_string =
		'$(document).ready(function()
		{
			...
			$("#storeDasboardPos").click(function()
			{
        		...
        		$(".loading").show();
        		$.ajax({
        			url: "ajax/machineDataDashboardSet.php",
        			type: "POST",
        			data: data,
        			cache: false,
        			timeout: 10000,
		...
		<div class="employer_full_content">
			<div class="form_holder">
							...
							?>
										<li>
											...
											<div class="curved_230">
												<div class="leftmenu">
													<h3 style="color:<?php echo $MACHINEDATA_QUICK_LINK[0][$section]; ?>;"><?php echo $text; ?></h3>
											<?php
				...
				<div class="testerDasboardQuickStat">
					<h2><span><?php echo 'Total Machine Data Acquiring'; ?></span></h2>
					<div class="testerDasboardHolder">
